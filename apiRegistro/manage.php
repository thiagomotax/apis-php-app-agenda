 ...
 switch($key)
   {
      ...
      case "create":
         ...
         $nome      = filter_var($_REQUEST['nome'], FILTER_SANITIZE_STRING, FILTER_FLAG_ENCODE_LOW);
         $email      = filter_var($_REQUEST['email'], FILTER_SANITIZE_STRING, FILTER_FLAG_ENCODE_LOW);
         $loginx      = filter_var($_REQUEST['loginx'], FILTER_SANITIZE_STRING, FILTER_FLAG_ENCODE_LOW);
         $senha      = filter_var($_REQUEST['senha'], FILTER_SANITIZE_STRING, FILTER_FLAG_ENCODE_LOW);


         // Attempt to run PDO prepared statement
         try {
          $sql  = "INSERT INTO usuarios(nomeUsuario, emailUsuario, loginUsuario, senhaUsuario) VALUES (:nome, :email, :loginx, :senha)";
          $stmt 	= $pdo->prepare($sql);
            $stmt->bindParam(':nome', $nome, PDO::PARAM_STR);
            $stmt->bindParam(':email', $email, PDO::PARAM_STR);
            $stmt->bindParam(':loginx', $loginx, PDO::PARAM_STR);
